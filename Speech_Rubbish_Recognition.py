...
GPIO.setup([27, 5, 13, 26], GPIO.OUT)        #27、5、13、26号引脚设置为输出模式
GPIO.output([27, 5, 13, 26], GPIO.LOW)       #初始化,27、5、13、26号引脚置低电平，默认状态下LED暗
...
class ASR:
    ...
    * 执行该函数，词条是自动往后排队添加的。   
    ...
    def led(self, pin):   
        count = 4
        while (count > 0):
            GPIO.output(pin, GPIO.HIGH)  #pin号引脚置高电平，此时LED灯被点亮
            time.sleep(0.2)             #持续0.3s
            GPIO.output(pin, GPIO.LOW)   #pin号引脚置低电平，此时LED灯熄灭
            time.sleep(0.2)             #持续0.3s
            count = count - 1
